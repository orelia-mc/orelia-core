...
- PvP決闘機能を追加(`/ol duel`)。決闘申請・承認、複数登録可能な専用アリーナ、HP0到達で実際には死なせず終了、勝者への少額報酬、勝敗数ランキング(`/ol duel ranking`)。決闘開始・アリーナ不足時は両者に通知するよう修正、決闘相手の切断以外の理由(第三者からの攻撃・環境ダメージ等)による実死亡でもセッションとアリーナを確実に解放、`/oladmin duelarena add|set`でPvP無効ワールドへの登録時に警告を表示。
- 一般プレイヤー向けコマンドをGUI優先に: `/ol pet`・`/ol mount`が素の実行でGUIを開くように変更(従来はチャット一覧が既定だった)。`/ol mount`にGUI画面を新設(`/ol pet`と同じ購入/召喚/送り返し操作)。`/ol job`にも転職画面を開くGUI導線を追加(従来は転職NPC経由のみ)。

- ネザースターのプレイヤー情報メニューから開くギルド/パーティー/フレンド/チャット設定画面に、メニューへ戻るボタンを追加。
