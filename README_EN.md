...
- Status GUI (`/ol status`) — split into 3 items (base: HP/SP/ATK/DEF/speed/SP recovery, plus current attack power factoring in the held weapon; crit: crit rate/crit damage; elemental damage: the 6 elements) instead of one cramped lore, with Japanese labels and the viewer's own skin on the head icon. Also shows money and the 6 accessory/relic equip slots (charm/ring/necklace/wing/earring/belt) on the same screen. Equipping/unequipping happens by clicking those slots directly (an empty slot shows a placeholder naming the part, and an item of the wrong part is refused). Equipment lives in a virtual slot set persisted to the database, independent of the player's real inventory, so it survives relog and server restarts.
