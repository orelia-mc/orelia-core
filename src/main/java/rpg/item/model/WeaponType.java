...
/**
 * The weapon categories in scope for this project (SOW section 6, extended for the
 * gathering jobs and for the mage's wand). Job and skill modules reference this enum to
 * express weapon restrictions and skill trees without redefining their own copy.
 *
 ...
 * their base item. WAND is the same idea applied to HOE: WAND is the mage's weapon and HOE
 * is the farmer's, both built on vanilla hoe materials. Which category a given
 * {@code ItemStack} belongs to is never inferred from its {@link Material} - only from the
 * id tag {@link rpg.item.service.WeaponFactory} stamps on it at creation (see
 * {@link rpg.item.service.WeaponIdentityService#dataOf}) - so reusing the same base
 * materials across two types never causes them to be confused with each other, and a plain
 * vanilla tool with no such tag is never treated as either.
 */
public enum WeaponType {
    ...
    WAND(List.of(Material.WOODEN_HOE, Material.STONE_HOE, Material.IRON_HOE, Material.GOLDEN_HOE, Material.DIAMOND_HOE, Material.NETHERITE_HOE));
